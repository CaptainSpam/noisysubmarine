...
interface SongDao {
    ...
    fun searchSongsForServerByName(serverId: Int, title: String): List<SongEntity>

    /** Get all stored songs across all configured servers. */
    @Query("SELECT * FROM songs")
    fun getAllSongsInAllServers(): List<SongEntity>
    ...
    fun searchSongsInAllServersByName(title: String): List<SongEntity>
    ...
    fun countSongsByArtist(serverId: Int, artistId: String): Int

    /** Get all songs by a given artist in a given server. */
    @Query("SELECT * FROM songs WHERE serverId = :serverId AND artistId = :artistId")
    fun getSongsByArtist(serverId: Int, artistId: String): List<SongEntity>
    ...
    fun countSongsInAlbum(serverId: Int, albumId: String): Int

    /** Get all songs in a given album in a given server. */
    @Query("SELECT * FROM songs WHERE serverId = :serverId AND albumId = :albumId")
    fun getSongsInAlbum(serverId: Int, albumId: String): List<SongEntity>
}